...
impl<T: Clone + Copy> Concat<T> for Vector<Vector<T>> {
    type Output = Vector<T>;

    fn concat(slice: &Self) -> Self::Output {
        let mut slice_iter = slice.iter();
        let initial_vector = match slice_iter.next() {
            Some(valid_slice_or_vector) => valid_slice_or_vector,
            None => return Vector::new(0),
        };
        let size = slice.iter().fold(0, |prev, curr| prev + curr.len());
        let mut result = Vector::with_capacity(size);
        result.extend_from_slice(initial_vector);

        slice.iter().skip(1).for_each(|slice| {
...
impl<T: Clone + Copy> Concat<T> for Vector<[T]>
where
    [T]: Sized,
{
    type Output = Vector<T>;
...
impl<T: Clone + Copy> Join<T> for Vector<Vector<T>> {
    type Output = Vector<T>;

    fn join(slice: &Self, sep: T) -> Vector<T> {
        let mut slice_iter = slice.iter();
        let initial_vector = match slice_iter.next() {
            Some(valid_slice_or_vector) => valid_slice_or_vector,
            None => return Vector::new(0),
        ...
        let size = slice
            .iter()
            .skip(1)
            .fold(initial_vector.len(), |prev, curr| prev + 1 + curr.len());
        let mut result = Vector::with_capacity(size);
        result.extend_from_slice(initial_vector);
...
impl<T: Clone + Copy> Join<T> for Vector<[T]>
where
    [T]: Sized,
{
    type Output = Vector<T>;
...
impl<T: Clone + Copy> Join<&[T]> for Vector<&[T]> {
    ...
    fn join(slice: &Self, sep: &[T]) -> Vector<T> {
        ...
        let size = slice
            .iter()
            .skip(1)
            .fold(initial_vector.len(), |prev, curr| {
                prev + sep.len() + curr.len()
            });
        let mut result = Vector::with_capacity(size);
        result.extend_from_slice(initial_vector);
...
mod tests {
    ...
    #[test]
    pub fn test_join() {
        let x: Vector<i32> = (0..10).collect();
        let y: Vector<i32> = (0..20).collect();
        let z: Vector<i32> = (0..30).collect();

        let p: i32 = 12;
        let collected = Vector::join(&Vector::from_array([x, y, z]), p);
        let expected: Vector<i32> = (0..10)
            .chain(12..13)
            .chain(0..20)
            .chain(12..13)
            .chain(0..30)
